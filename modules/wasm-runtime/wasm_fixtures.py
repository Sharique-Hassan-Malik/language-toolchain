"""WebAssembly modules used by the tests, importable by name.

This was `tests/fixtures.py` and was imported as `tests.fixtures`, which
resolves to whichever `tests` package is nearest — and the repository root
has one too, once every module is collected in a single run. A uniquely
named module at the module root has one meaning in both contexts.
"""
